...
export default function CustomForm({ inputs, onSubmitCB, validationSchema }: CustomFormInputs) {
                    ...
                    {inputs.map((input) => {
                        return <>
                            <label key={input.name}>
                                {input.name}:
                                <Field
                                    list={input.type === 'list' ? `dataList-${input.name}` : ''}
                                    name={input.name}
                                    placeholder={input.placeholder}
                                    className={touched[input.name] && errors[input.name] ? 'invalid' : ''}
                                />
                                {input.type === 'list'
